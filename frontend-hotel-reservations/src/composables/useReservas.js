...
export function useReservas() {
  ...
  const reservaOptions = computed(() => {
    const options = reservas.value
      ...
      .map((r) => ({
        label: `Reserva #${r.Id} - ${r.ClienteNombre || "Desconocido"} - Hab. ${r.HabitacionNumero || "N/A"}`,
        value: r.Id,
        reserva: r,
      }))
  ...
  const formDisabled = computed(() => {
    const habitacionValue = typeof reservaForm.value.HabitacionNumero === "object" && reservaForm.value.HabitacionNumero?.value
      ? reservaForm.value.HabitacionNumero.value
      : reservaForm.value.HabitacionNumero
    const clienteIdValue = typeof reservaForm.value.ClienteId === "object" && reservaForm.value.ClienteId?.value
  ...
  const canRegisterArrival = computed(() => {
    ...
    const result = reserva && !reserva.EstaElClienteEnHostal && !reserva.EstaCancelada && today >= DateTime.fromISO(reserva.FechaEntrada).toISODate()
    console.log("canRegisterArrival:", { reservaId: selectedReservaAccion.value?.value, today, fechaEntrada: reserva?.FechaEntrada, result })
    return result
  })
  ...
  const canCancelReserva = computed(() => {
    ...
    const today = DateTime.now().toISODate()
    const result = reserva && !reserva.EstaElClienteEnHostal && !reserva.EstaCancelada && today <= DateTime.fromISO(reserva.FechaEntrada).toISODate()
    console.log("canCancelReserva:", { reservaId: selectedReservaAccion.value?.value, today, fechaEntrada: reserva?.FechaEntrada, result })
    return result
  })
  ...
  const canChangeRoom = computed(() => {
    ...
    const today = DateTime.now().toISODate()
    const result = reserva && !reserva.EstaElClienteEnHostal && !reserva.EstaCancelada && today <= DateTime.fromISO(reserva.FechaEntrada).toISODate()
    console.log("canChangeRoom:", { reservaId: selectedReservaAccion.value?.value, today, fechaEntrada: reserva?.FechaEntrada, result })
    return result
  })
  ...
  const calcularNoches = computed(() => {
    ...
    console.log("calcularNoches:", { entrada: reservaForm.value.FechaEntrada, salida: reservaForm.value.FechaSalida, noches })
    return noches
  })

  const clienteEsVip = computed(() => {
    if (!reservaForm.value.ClienteId) return false
    const clienteId = typeof reservaForm.value.ClienteId === "object" ? reservaForm.value.ClienteId.value : reservaForm.value.ClienteId
  ...
  const showNotification = (message, type = "positive") => {
    $q.notify({
      message,
      color: type === "positive" ? "green" : type === "warning" ? "yellow" : "red",
      position: "top",
      timeout: 3000,
      icon: type === "positive" ? "check_circle" : type === "warning" ? "warning" : "error",
    })
  }
  ...
  const fetchClientes = async () => {
    try {
      ...
      clienteOptions.value = Array.isArray(response.data) ? response.data.map((cliente) => ({
      ...
      showNotification("Error al cargar clientes: " + (error.response?.data?.message || error.message), "negative")
    }
  }
  ...
  const fetchHabitacionesDisponibles = async () => {
    if (!reservaForm.value.FechaEntrada || !reservaForm.value.FechaSalida) {
      ...
      showNotification("Seleccione fechas de entrada y salida para cargar habitaciones disponibles", "warning")
      return
    }
    try {
      ...
      habitacionOptions.value = Array.isArray(response.data) ? response.data.map((num) => ({
      ...
      habitacionOptions.value = []
      showNotification("Error al cargar habitaciones disponibles: " + (error.response?.data?.message || error.message), "negative")
    }
  }
  ...
  const fetchReservas = async () => {
    ...
    try {
      ...
      reservas.value = Array.isArray(response.data.data) ? response.data.data.map((reserva) => ({
      ...
      reservas.value = []
      showNotification("Error al cargar reservas: " + (error.response?.data?.message || error.message), "negative")
    } finally {
      loading.value = false
    }
  }
  ...
  const fetchReservasActivas = async () => {
    ...
    try {
      ...
      reservasActivas.value = Array.isArray(response.data.data) ? response.data.data.map((reserva) => ({
        Id: reserva.Id?.toString() || "N/A",
        FechaReservacion: reserva.FechaReservacion
          ? DateTime.fromISO(reserva.FechaReservacion).toFormat("yyyy-MM-dd")
          : "N/A",
      ...
      showNotification("Error al cargar reservas activas: " + (error.response?.data?.message || error.message), "negative")
    } finally {
      loading.value = false
    }
  }
  ...
  const crearReserva = async () => {
    loading.value = true
    try {
      const habitacionValue = typeof reservaForm.value.HabitacionNumero === "object" 
        ? reservaForm.value.HabitacionNumero?.value 
        : reservaForm.value.HabitacionNumero
      const clienteId = typeof reservaForm.value.ClienteId === "object" 
        ? reservaForm.value.ClienteId?.value 
        : reservaForm.value.ClienteId
      const payload = {
  ...
  const cancelarReserva = async () => {
    ...
    try {
      console.log("Cancelling reserva:", { id: selectedReservaAccion.value.value, motivo: motivoCancelacion.value })
      const response = await axios.put(`${apiUrl}/Reservas/${selectedReservaAccion.value.value}/cancelar`, `"${motivoCancelacion.value}"`, {
  ...
  const registrarLlegada = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${apiUrl}/Reservas/${selectedReservaAccion.value.value}/registrar-llegada`)
  ...
  const cambiarHabitacion = async () => {
    if (!selectedReservaAccion.value?.value || !nuevaHabitacion.value || !/^0[1-3][1-5]$/.test(nuevaHabitacion.value)) {
      showNotification("Por favor, ingrese un número de habitación válido (formato 0XY)", "negative")
      return
    }
    try {
      console.log("Changing habitacion:", { reservaId: selectedReservaAccion.value.value, nuevaHabitacion: nuevaHabitacion.value })
      const response = await axios.put(`${apiUrl}/Reservas/${selectedReservaAccion.value.value}/cambiar-habitacion`, `"${nuevaHabitacion.value}"`, {
        headers: { "Content-Type": "application/json" },
      })
  ...
  const selectReserva = async (id) => {
    try {
      ...
      const cliente = clienteOptions.value.find((c) => c.value === reserva.ClienteId) || {
      ...
      reservaForm.value = {
        ...
        ClienteId: cliente,
        HabitacionNumero: { label: `Habitación ${reserva.HabitacionNumero}`, value: reserva.HabitacionNumero },
      }
      ...
      showNotification("Error al obtener reserva: " + (error.response?.data?.message || error.message), "negative")
    }
  }

  const selectReservaFromList = (reserva) => {
    selectedReservaAccion.value = {
      label: `Reserva #${reserva.Id} - ${reserva.ClienteNombre || "Desconocido"} - Hab. ${reserva.HabitacionNumero || "N/A"}`,
      value: reserva.Id,
      reserva,
    }
